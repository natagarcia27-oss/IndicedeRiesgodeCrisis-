import streamlit as stimport pandas as pdimport plotly.graph_objects as gofrom datetime import datetime
...
st.set_page_config(page_title="Índice de Riesgo de Crisis",page_icon="📊",layout="wide")
...
def obtener_escenario_dominante(estable,creciente,critico):
...
def generar_resumen(irc,iaam,escenario):
...
def generar_recomendaciones(escenario,iaam):
...
archivo = st.sidebar.file_uploader("Cargar matriz diligenciada",type=["xlsx"])

procesar = st.sidebar.button("Procesar evaluación")
...
st.title("Índice de Riesgo de Crisis ante manifestaciones sociales violentas")

st.caption("Sistema de monitoreo estratégico")
